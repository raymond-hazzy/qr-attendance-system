...
const LecturerScanner = () => {
  ...
  useEffect(() => {
    if (Html5QrcodeScanner && !scannerRef.current && isScanning && !scannerInitialized) {
      initializeScanner();
    }
  ...
  const handleSuccessfulScan = async (encryptedData: string) => {
    ...
    try {
      ...
      setSuccessMessage(`Attendance marked for ${response.attendance.fullName}!`);
    ...
    <div className="min-h-screen bg-[var(--color-light-grey-100)] p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6 border border-[var(--color-light-grey-200)]">
          <h2 className="text-xl font-semibold mb-4 text-[var(--color-navy-blue-700)]">Scan Student QR Code</h2>
          ...
          {lastScannedStudent && (
            <div className="bg-[var(--color-navy-blue-50)] p-4 rounded-lg mt-4 border border-[var(--color-navy-blue-100)]">
              <h3 className="font-semibold text-[var(--color-navy-blue-800)] mb-2">Last Scanned Student:</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                <div><span className="font-medium">Name:</span> {lastScannedStudent.fullName}</div>
                <div><span className="font-medium">Matric No:</span> {lastScannedStudent.matricNo}</div>
                <div><span className="font-medium">Course:</span> {lastScannedStudent.courseCode}</div>
                <div><span className="font-medium">Total Scans:</span> 
